...
(function () {
  ...
  function renderMOSDashboard(data, options) {
        if (!data || !data.length) {
            console.warn('renderMOSDashboard: данные пусты');
        ...
        if (!mos.length) {
            console.warn('renderMOSDashboard: не удалось разобрать строки МО', data);
    ...
    <script>
        ...
        function renderHtmlContent(text) {
            ...
            let safe = text;
            if (!safe.includes('<br') && !safe.includes('<p') && !safe.includes('<div') && !safe.includes('<li')) {
                safe = safe.replace(/\\n/g, '<br>');
            }
            return safe;
        }
